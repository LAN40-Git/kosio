...
kosio::runtime::io::IoUring::~IoUring() {
    t_ring = nullptr;
    io_uring_queue_exit(&ring_);
}

auto kosio::runtime::io::IoUring::peek_batch(std::span<io_uring_cqe*> cqes) -> std::size_t {
    return io_uring_peek_batch_cqe(&ring_, cqes.data(), cqes.size());
}

auto kosio::runtime::io::IoUring::peek_cqe(io_uring_cqe** cqe) -> int {
    return io_uring_peek_cqe(&ring_, cqe);
}

void kosio::runtime::io::IoUring::wait(std::optional<time_t> timeout_ms) {
    // TODO: 记录错误
...
void kosio::runtime::io::IoUring::pend_submit() {
    submit_tick_++;
...
void kosio::runtime::io::IoUring::pend_submit_batch(std::size_t count) {
    submit_tick_ += count;
    if (submit_tick_ >= submit_interval_) {
        submit();
    }
...
void kosio::runtime::io::IoUring::submit() {
    submit_tick_ = 0;
