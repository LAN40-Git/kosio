...
#include "kosio/runtime/scheduler/multi_thread/worker.h"
#include "kosio/common/util/thread.h"
...
kosio::runtime::scheduler::multi_thread::Handle::~Handle() {
    shared_.close();
...
void kosio::runtime::scheduler::multi_thread::Handle::schedule_task(std::coroutine_handle<> task) {
    shared_.schedule_remote(task);
}

void kosio::runtime::scheduler::multi_thread::Handle::close() const {
    shared_.close();
}

void kosio::runtime::scheduler::multi_thread::Handle::wait() {
    for (auto &thread : threads_) {
        if (thread.joinable()) {
            thread.join();
        }
