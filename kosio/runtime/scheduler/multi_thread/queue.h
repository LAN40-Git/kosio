...
public:
    [[nodiscard]]
    auto remaining_slots() const -> std::size_t {
        auto [steal, _] = unpack(head_.load(std::memory_order::acquire));
        auto tail = tail_.load(std::memory_order::acquire);
        assert(runtime::detail::LOCAL_QUEUE_CAPACITY >= static_cast<std::size_t>(tail - steal));
        return runtime::detail::LOCAL_QUEUE_CAPACITY - static_cast<std::size_t>(tail - steal);
    }
    ...
    void push_back_or_overflow(std::coroutine_handle<> task, GlobalQueue &global_queue) {
        ...
        while (true) {
            ...
            if (tail - steal < static_cast<uint32_t>(runtime::detail::LOCAL_QUEUE_CAPACITY)) {
    ...
    auto steal_into(LocalQueue &dst) -> std::optional<std::coroutine_handle<>> {
        ...
        if (dst_tail - steal > static_cast<uint32_t>(runtime::detail::LOCAL_QUEUE_CAPACITY / 2)) {
            return result;
        }
...
private:
    ...
    auto push_overflow(std::coroutine_handle<>  &task,
        ...
        static constexpr auto NUM_TASKS_TAKEN{
            static_cast<uint32_t>(runtime::detail::LOCAL_QUEUE_CAPACITY / 2)};

        assert(tail - head == runtime::detail::LOCAL_QUEUE_CAPACITY);
...
private:
    static constexpr std::size_t MASK = runtime::detail::LOCAL_QUEUE_CAPACITY - 1;
