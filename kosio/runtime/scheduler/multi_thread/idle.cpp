#include "kosio/runtime/scheduler/multi_thread/idle.h"
#include "kosio/common/debug.h"

kosio::runtime::scheduler::multi_thread::IdleState::IdleState(
    std::size_t num_workers)
    : state_(num_workers << WORKING_SHIFT) {}

auto kosio::runtime::scheduler::multi_thread::IdleState::num_searching(
    std::memory_order order) const -> std::size_t {
    return state_.load(order) & SEARCHING_MASK;
}

auto kosio::runtime::scheduler::multi_thread::IdleState::num_working(
    std::memory_order order) const -> std::size_t {
    return (state_.load(order) & WORKING_MASK) >> WORKING_SHIFT;
}

auto kosio::runtime::scheduler::multi_thread::IdleState::num_working_and_searching(
...
void kosio::runtime::scheduler::multi_thread::IdleState::inc_num_searching(std::memory_order order) {
    state_.fetch_add(1, order);
}

auto kosio::runtime::scheduler::multi_thread::IdleState::dec_num_searching() -> bool {
    auto prev = state_.fetch_sub(1, std::memory_order::seq_cst);
    return (prev & SEARCHING_MASK) == 1;
}

void kosio::runtime::scheduler::multi_thread::IdleState::wake_up_one(std::size_t num_searching) {
    // 增加一个工作中线程：(1 << WORKING_SHIFT)
    // 增加 num_searching 个搜索线程
    state_.fetch_add(num_searching | (1 << WORKING_SHIFT), std::memory_order::seq_cst);
}

auto kosio::runtime::scheduler::multi_thread::IdleState::dec_num_working(bool is_searching) -> bool {
...
kosio::runtime::scheduler::multi_thread::Idle::Idle(std::size_t num_workers)
    : state_(num_workers)
    , num_workers_(num_workers) {}

auto kosio::runtime::scheduler::multi_thread::Idle::worker_to_notify() -> std::optional<std::size_t> {
...
auto kosio::runtime::scheduler::multi_thread::Idle::transition_worker_to_sleeping(std::size_t worker,
...
auto kosio::runtime::scheduler::multi_thread::Idle::transition_worker_to_searching() -> bool {
...
auto kosio::runtime::scheduler::multi_thread::Idle::transition_worker_from_searching() -> bool {
    return state_.dec_num_searching();
}

auto kosio::runtime::scheduler::multi_thread::Idle::remove(std::size_t worker) -> bool {
    std::lock_guard lock(mutex_);
...
auto kosio::runtime::scheduler::multi_thread::Idle::contains(std::size_t worker) -> bool {
    std::lock_guard lock(mutex_);
    return sleeping_workers_.contains(worker);
}

auto kosio::runtime::scheduler::multi_thread::Idle::notify_should_wakeup() -> bool {
