...
public:
    [[nodiscard]]
    static auto parse(std::string_view ip) -> std::expected<Ipv4Addr, std::error_code> {
        uint32_t addr;
...
public:
    [[nodiscard]]
    friend auto
    operator==(const Ipv4Addr& lhs, const Ipv4Addr& rhs) noexcept -> bool {
...
{
template <>
class formatter<coruring::socket::net::Ipv4Addr> {
public:
    ...
    auto format(coruring::socket::net::Ipv4Addr addr, auto &context) const noexcept {
        return format_to(context.out(), "{}", addr.to_string());
    }
};

template <>
class formatter<coruring::socket::net::Ipv6Addr> {
public:
    constexpr auto parse(format_parse_context &context) {
        auto it{context.begin()};
        auto end{context.end()};
    ...
    auto format(const coruring::socket::net::Ipv6Addr &addr, auto &context) const noexcept {
        return format_to(context.out(), "{}", addr.to_string());
    }
};

template <>
class formatter<coruring::socket::net::SocketAddr> {
public:
    constexpr auto parse(format_parse_context &context) {
        auto it{context.begin()};
        auto end{context.end()};
    ...
    auto format(const coruring::socket::net::SocketAddr &addr, auto &context) const noexcept {
        return format_to(context.out(), "{}", addr.to_string());
    }
};
} // namespace std